package cn.lwjzt.core.common.json;
...
public class JsonWriter implements Closeable, Flushable
{
    ...
    /**
     * @see JsonWriter#objectToJson(Object, Map)
    ...
    public JsonWriter(OutputStream out, Map<String, Object> optionalArgs)
    {
        ...
        {
            for (Entry<Class, JsonClassWriterBase> entry : customWriters.entrySet())
            {
    ...
    private JsonClassWriterBase forceGetCustomWriter(Class c)
    {
        ...
        for (Entry<Class, JsonClassWriterBase> entry : writers.entrySet())
        {
    ...
    {
        final Iterator<Entry<Class, List<Field>>> i = fieldSpecifiers.entrySet().iterator();
        int minDistance = Integer.MAX_VALUE;
        List<Field> fields = null;

        while (i.hasNext())
        {
            final Entry<Class, List<Field>> entry = i.next();
...
//        boolean typeWritten = showType && !(Object[].class == arrayType);    // causes IDE warning in NetBeans 7/4 Java 1.7
    ...
    public void writeObject(final Object obj, boolean showType, boolean bodyOnly) throws IOException
    {
        ...
        {   // Reflectively use fields, skipping transient and static fields
            ...
            for (Entry<String, Field> entry : classFields.entrySet())
            {
                final String fieldName = entry.getKey();
